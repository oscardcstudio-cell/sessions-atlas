...
interface ChatMessageComponentProps {
  ...
  stats?: { inputTokens: number; outputTokens: number; costUsd: number } | null;
}

function TokenWheel({ fraction }: { fraction: number }) {
  const r = 5.5;
  ...
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" style={{ transform: "rotate(-90deg)", flexShrink: 0 }}>
      <circle cx="8" cy="8" r={r} fill="none" stroke="#333" strokeWidth="2.5" />
      <circle cx="8" cy="8" r={r} fill="none" stroke="#c96442" strokeWidth="2.5"
...
export function ChatMessageComponent({ message, modelName, stats }: ChatMessageComponentProps) {
  const isUser = message.role === "user";

  if (isUser) {
    return (
      <div className="flex justify-end mb-4 px-1">
        <div className="max-w-[75%] rounded-[20px] rounded-br-[5px] px-4 py-3 bg-[#2f2f2f]">
          <pre className="whitespace-pre-wrap text-sm leading-relaxed text-[#ececec]" style={{ fontFamily: "inherit" }}>
            {message.content}
          </pre>
        </div>
      </div>
  ...
  const shortModel = modelName
    ...
    : null;
  const fraction = stats ? stats.inputTokens / 200_000 : 0;

  return (
    <div className="flex gap-3 mb-6 items-start px-1">
      <div className="w-7 h-7 rounded-full flex items-center justify-center flex-none text-white text-xs font-semibold mt-0.5 bg-[#c96442]">
        C
      </div>
      <div className="flex-1 min-w-0">
        ...
        {(shortModel || stats) && (
          <div className="flex items-center gap-2.5 mt-3">
            {shortModel && (
              <span className="text-[10.5px] text-[#6a6a6a] font-medium">{shortModel}</span>
            )}
            {stats && (
              <>
                <TokenWheel fraction={fraction} />
                <span className="text-[10.5px] text-[#6a6a6a]">
                  {fmtK(stats.inputTokens)}↑ {fmtK(stats.outputTokens)}↓
                </span>
                <span className="text-[10.5px] text-[#6a6a6a]">${stats.costUsd.toFixed(4)}</span>
              </>
            )}
          </div>
        )}
    ...
    <div className="flex gap-3 mb-6 items-start px-1">
      ...
      <div className="flex items-center gap-1.5 pt-2">
        {[0, 1, 2].map((i) => (
          <div
            key={i}
            className="w-1.5 h-1.5 rounded-full animate-bounce bg-[#6a6a6a]"
